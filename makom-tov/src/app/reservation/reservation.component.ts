...
export class ReservationComponent implements OnInit {
  ...
  // ngOnInit() {
  //   //this.sharedService.getUserDetails("111",'333');
  // }
  ...
  OnItemDeSelect(item: any) {
    console.log(item);
    var objIndex = this.dropdownList.findIndex((obj => obj.id == item.id));
  ...
  editDogs(event){
    if(event.action === 1){
      var objIndex = this.selectedItems.findIndex((obj => obj.id == event.id));
